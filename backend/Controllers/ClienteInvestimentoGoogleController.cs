...
namespace backend.Controllers
{
    ...
    public class ClienteInvestimentoGoogleController : ControllerBase
    {
        private readonly AppDbContext _context;

        public ClienteInvestimentoGoogleController(AppDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            var query = _context.ClientesInvestimentosGoogle.AsQueryable();
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] ClienteInvestimentoGoogle invest)
        {
            _context.ClientesInvestimentosGoogle.Add(invest);
            await _context.SaveChangesAsync();
            return CreatedAtAction(nameof(GetByCliente), new { idCliente = invest.IdCliente }, invest);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] ClienteInvestimentoGoogle invest)
        {
        ...
        {
            var invest = await _context.ClientesInvestimentosGoogle.FindAsync(id);
            if (invest == null) return NotFound();
            _context.ClientesInvestimentosGoogle.Remove(invest);
            await _context.SaveChangesAsync();
            return NoContent();
        }
    }
